...
#define YAS_ACC_KERNEL_VERSION                                       "4.18.910"
...
//S [CCI]Ginger modified for factory test
unsigned char g_accel_product_id = 0;
static ssize_t
yas_acc_ping(struct device *dev, struct device_attribute *attr,
	char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct yas_acc_private_data *data = input_get_drvdata(input);
	data->driver->set_enable(1);
...
static DEVICE_ATTR(ping, S_IRUGO|S_IWUSR|S_IWGRP, yas_acc_ping, NULL);
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (err < 0)
	{
		printk("[CCI]yas_kionix_accel_probe: sysfs_create_dev group failed[%s]\n",
					data->input->name);
	}
        //E [CCI]Ginger modified for factory test
...
//S [CCI]Ginger modified for MSM8226 DTS
static struct of_device_id yas_accel_dts_table[] = {
		{ .compatible  = "qcom,accelerometer",},
		{ },
};
//E [CCI]Ginger modified for MSM8226 DTS
...
static struct i2c_driver yas_acc_driver = {
	.driver = {
		.name = "accelerometer",
		.owner = THIS_MODULE,
		.of_match_table = yas_accel_dts_table,  //[CCI]Ginger modified for MSM8226 DTS
	},
	.probe = yas_acc_probe,
